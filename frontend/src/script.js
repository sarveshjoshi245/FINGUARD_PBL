        ...
        formArea.innerHTML = `
            <div class="input-with-voice">
                <div class="form-group">
                    ...
                    <input type="text" id="custName" value="${savedName}" placeholder="Enter your full name">
                </div>
                <button type="button" class="voice-btn" id="voiceNameBtn" onclick="startVoiceRecognition('custName', this)" title="Fill with Voice">🎤</button>
            </div>
 ...
 */
/**
 * Advanced Signature Comparison using OpenCV.js ORB Feature Matching.
 * Effectively handles scaling, rotation, and minor variations in stroke pressure.
 */
async function compareSignatures(img1Src, img2Src) {
    ...
    scoreEl.innerText = '⏳ AI Feature Extraction...';
    scoreEl.style.color = '#002147';
    if (detailEl) detailEl.innerText = 'Extracting structural landmarks using ORB...';

    // Fallback if OpenCV isn't loaded yet
    if (!window.cv || !cv.Mat) {
        console.warn("OpenCV not ready, falling back to basic comparison.");
        return runResembleFallback(img1Src, img2Src);
    }

    // Helper to load image to CV Mat
    const loadImageToMat = (src) => new Promise((resolve) => {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => {
            const canvas = document.createElement('canvas');
    ...
    try {
        const [mat1, mat2] = await Promise.all([loadImageToMat(img1Src), loadImageToMat(img2Src)]);

        // 1. Initialize ORB detector
        const orb = new cv.ORB(500); // 500 features is plenty for signatures
        ...
        // 4. Filter "good" matches (distance check)
        let goodMatches = 0;
        let totalDist = 0;
        const matchData = [];
        for (let i = 0; i < matches.size(); i++) {
            const match = matches.get(i);
            if (match.distance < 45) { // Threshold for Hamming distance
                goodMatches++;
        ...
        // 5. Calculate Score
        // Base score on ratio of good matches to the average number of features found
        const avgFeatures = (keypoints1.size() + keypoints2.size()) / 2;
        const matchRatio = goodMatches / Math.max(1, avgFeatures);
        
        // Logarithmic scaling to give a human-friendly 0-100 score
        let matchScore = Math.min(100, Math.round(matchRatio * 200)); 
        
        // Bonus for very strong consistency
        if (matchScore > 30 && goodMatches > 15) matchScore += 10;
        matchScore = Math.min(100, matchScore);

        console.log(`ORB Analysis: ${goodMatches} good matches found out of ${avgFeatures} avg features. Score: ${matchScore}`);

        // Update UI
        scoreEl.innerText = `AI Match Score: ${matchScore}%`;
        
        if (detailEl) {
            detailEl.innerText = `Detected ${Math.round(avgFeatures)} structural points | ${goodMatches} strong matches found.`;
        }

        // Visualize matches on a temporary canvas if possible
        if (diffImg) {
            const output = new cv.Mat();
            cv.drawMatches(mat1, keypoints1, mat2, keypoints2, matches, output);
            const canvas = document.createElement('canvas');
        ...
        const PASS_THRESHOLD = 35; // Lower for structural matching as it's more strict
        ...
        if (matchScore >= PASS_THRESHOLD) {
            ...
            if (detailEl) detailEl.innerText += ' | ⚠ Structural variation detected.';
            if (nextBtn) {
        ...
        // Cleanup
        mat1.delete(); mat2.delete(); descriptors1.delete(); descriptors2.delete();
        keypoints1.delete(); keypoints2.delete(); matches.delete(); orb.delete(); bf.delete();

    } catch (err) {
        console.error('Advanced comparison error:', err);
        scoreEl.innerText = '❌ Analysis Error. Review manually.';
        if (nextBtn) {
            nextBtn.style.display = "inline-block";
            nextBtn.onclick = () => { currentStep++; renderStep(); saveDraft(); };
        }
...
/**
 * Fallback to resemble.js if OpenCV isn't available
 */
async function runResembleFallback(img1Src, img2Src) {
    const scoreEl = document.getElementById('sig-score');
    resemble(img1Src).compareTo(img2Src).onComplete((data) => {
        const score = Math.round(100 - data.misMatchPercentage);
        scoreEl.innerText = `Match Score: ${score}% (Fallback)`;
        tempData.signatureMatchScore = score;
        const nextBtn = document.getElementById("nextBtn");
...
function startOnboardingForm() {
    // Hide intro screen and show the form
    const introScreen = document.getElementById('aiIntroScreen');
    ...
    console.log("🚀 Onboarding form started");
}
...
function showCustomerSection(id) {
    // Explicitly hide only the known customer portal sections
    ['home', 'openAccount', 'loan', 'fd'].forEach(function(sectionId) {
    ...
    document.querySelectorAll('#customerPortal .sidebar button').forEach(function(btn) {
        btn.classList.remove('sidebar-active');
    });
    var activeBtn = document.querySelector('#customerPortal .sidebar button[onclick*="' + id + '"]');
...
async function updateKpiDisplay() {
    try {
        ...
        const totalEl = document.getElementById('nudgeTotal');
        if (totalEl) totalEl.textContent = stats.totalApplications || 0;

        // Update Success Rate
        const successRate = stats.successRate ? Math.round(stats.successRate) : 0;
        const successRateEl = document.querySelector('.eng-nudge-metric.eng-nudge-success .eng-nudge-rate');
        if (successRateEl) successRateEl.textContent = `▲ ${successRate}% Success`;

        // Update retained users
        const successCountEl = document.querySelector('.eng-nudge-metric.eng-nudge-success .eng-nudge-val');
        if (successCountEl && stats.totalApplications) {
            const retained = Math.round(stats.totalApplications * (successRate / 100));
            successCountEl.textContent = retained;
        }

        // Update dropped users
        const droppedEl = document.querySelector('.eng-nudge-metric.eng-nudge-drop .eng-nudge-val');
        if (droppedEl && stats.totalApplications) {
            const dropped = stats.totalApplications - (stats.approvedCount || 0);
            droppedEl.textContent = dropped;
        }

        // Update bar
        const barFill = document.querySelector('.eng-nudge-bar-fill');
        if (barFill) {
            barFill.style.width = `${successRate}%`;
            barFill.textContent = `${successRate}% Retained`;
        }
    } catch (err) {
        console.warn('Failed to fetch KPI stats:', err);
    }
}
...
function renderTopQueries() {
    ...
    // Fetch top queries from backend (for now use mock data)
    const mockQueries = [
        { query: 'How long does this take?', count: 48 },
        { query: 'What documents do I need?', count: 42 },
        { query: 'Is my data secure?', count: 35 },
        { query: 'Can I use Aadhaar?', count: 28 }
    ];
    
    const maxCount = Math.max(...mockQueries.map(q => q.count));
    container.innerHTML = mockQueries.map((q, i) => {
...
function renderAnalyticsTab() {
    renderFunnelChartFromStats();
    renderTimeChart();
    renderFrictionPoints();
}

async function renderFunnelChartFromStats() {
    try {
        const res = await fetch(`${API_BASE}/api/applications/stats/summary`);
        ...
        // Calculate funnel from real data
        const total = stats.totalApplications || 100;
        const approved = stats.approvedCount || 0;
        const pending = stats.pendingCount || 0;
        
        // Simulate stages based on real data
        const labels = ['Started', 'Documents', 'Verification', 'Approved'];
        const percentages = [
            100,
            Math.round((total / Math.max(total, 1)) * 100),
            Math.round((approved / Math.max(total, 1)) * 85),
            Math.round((approved / Math.max(total, 1)) * 100)
        ];

        // Find biggest drop
        let maxDrop = 0, maxDropIdx = 0;
        for (let i = 1; i < percentages.length; i++) {
            const drop = percentages[i - 1] - percentages[i];
        ...
        insightDiv.innerHTML = `<span class=\"insight-icon\">💡</span> Biggest drop-off: <strong>${labels[maxDropIdx]}</strong> — <strong>${maxDrop}%</strong> attrition.`;
    } catch (err) {
        console.warn('Failed to fetch funnel stats:', err);
    }
}
...
function initAutosave() {
    ...
    console.log("💾 Autosave Enabled: Draft will be saved every 20 seconds");
    
    // Save draft every 20 seconds while user is in the onboarding steps
    autosaveInterval = setInterval(() => {
        const customerPortal = document.getElementById('customerPortal');
        const stepContent = document.getElementById('stepContent');
        
        // Only autosave if we're in the customer portal and past the intro screen
        if (customerPortal && customerPortal.style.display !== 'none' && stepContent && stepContent.style.display !== 'none') {
            if (tempData.mobile && currentStep > 0 && currentStep < 6) {
                saveDraft();
            }
        }
    }, AUTOSAVE_INTERVAL);
...
window.addEventListener('DOMContentLoaded', function () {
    // Wrap each call so one failure doesn't block the portal from showing
    try { renderApplicationList(); } catch(e) { console.warn('renderApplicationList:', e); }
    try { renderEngagementTab(); } catch(e) { console.warn('renderEngagementTab:', e); }
    try { initDropoffPrevention(); } catch(e) { console.warn('initDropoffPrevention:', e); }
    try { initSessionTimer(); } catch(e) { console.warn('initSessionTimer:', e); }
    try { initAutosave(); } catch(e) { console.warn('initAutosave:', e); }
    showPortal('customerPortal'); // Always runs last — shows the page
});
/* ================= VOICE RECOGNITION ================= */
function startVoiceRecognition(inputId, btn) {
    ...
    // Map UI language to Speech recognition codes
    ...
    btn.innerHTML = '🔴'; // recording indicator
    const originalPlaceholder = input.placeholder;
    input.placeholder = "Listening... (Please speak clearly)";

    recognition.onresult = (event) => {
        let transcript = event.results[0][0].transcript;
        
        // Remove trailing punctuation like periods, exclamation marks, etc.
        transcript = transcript.replace(/[.!?]+$/, "").trim();

        // Clean up the transcript: capitalize first letters of names
        const formattedName = transcript.split(' ')
            .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
            .join(' ');
        
        input.value = formattedName;
        showNotification("Name filled successfully!");
    };

    recognition.onerror = (event) => {
        console.error("Speech Recognition Error:", event.error);
        if (event.error === 'not-allowed') {
            showNotification("Microphone access denied. Please allow in browser settings.", true);
        } else {
            showNotification("Voice capture failed. Please try again.", true);
        }
    };
